...
const Navbar: React.FC = () => {
  ...
  const dashboardLinks: DashboardLink[] = [
    ...
    { name: 'Mordor', href: '#', icon: CodeBracketIcon },
    { name: 'Informator', href: '#', icon: BookOpenIcon },
    { name: 'Gutenberg', href: '#', icon: PrinterIcon },
  ...
  useEffect(() => {
    const handleClickOutside = (event: MouseEvent) => {
      if (
        ...
        !extraMenuButtonRef.current.contains(event.target as Node)
      ) {
      ...
      <nav className={`fixed top-4 left-1/2 -translate-x-1/2 w-[90%] max-w-7xl z-50 rounded-full p-2 transform-gpu transition-all duration-300 ${navStyles}`}>
        <div className="flex items-center justify-between px-6">
          ...
          <div className="hidden md:flex items-center gap-x-6 font-inter">
            {navLinks.map((link) => (
              <a key={link.name} href={link.href} className="text-sm leading-6 text-gray-900 hover:text-gray-700 transition-colors">
                {link.name}
              </a>
            ))}
          ...
          <div className="hidden md:block">
            <a href="/join" className="rounded-full bg-slate-800 px-6 py-2 text-sm font-medium font-inter text-white shadow-sm hover:bg-slate-700 transition-colors">
              Dołącz do nas
            </a>
          </div>
          ...
          <div className="origin-top rounded-3xl bg-white shadow-lg border border-black/10 p-6">
            <div className="flex flex-wrap justify-center gap-4">
              {dashboardLinks.map((link) => (
                <DashboardLinkComponent key={link.name} link={link} />
              ))}
            </div>
          </div>
        </div>
        ...
        <div className="fixed inset-0 z-50 bg-white md:hidden overflow-y-auto">
          ...
          <div className="min-h-[calc(100vh-80px)] flex flex-col">
            <div className="flex-1 px-8 py-12">
              <div className="text-center space-y-8">
                <div className="space-y-6">
                  {navLinks.map((link) => (
                    <a key={link.name} href={link.href} onClick={closeMobileMenu} className="block text-2xl font-semibold text-gray-900 hover:text-gray-600 transition-colors py-2">
                      {link.name}
                    </a>
                  ))}
                </div>
                ...
                <div className="space-y-6">
                  <button onClick={() => setIsMobileExtraMenuOpen(!isMobileExtraMenuOpen)} className="flex items-center justify-center gap-2 text-2xl font-semibold text-gray-900 hover:text-gray-600 transition-colors py-2 mx-auto">
                    Linki
                    <ChevronDownIcon className={`h-5 w-5 transition-transform duration-200 ${isMobileExtraMenuOpen ? 'rotate-180' : ''}`} />
                  </button>

                  <Transition
                    show={isMobileExtraMenuOpen}
                    enter="transition ease-out duration-300"
                  ...
                  >
                    <div className="mt-6">
                      <div className="flex flex-wrap justify-center gap-4 max-w-sm mx-auto">
                         {dashboardLinks.map((link) => (
                          <DashboardLinkComponent key={link.name} link={link} onClick={closeMobileMenu} />
                        ))}
                      </div>
                    </div>
                  </Transition>
            ...
            <div className="p-6 border-t border-gray-100 text-center">
              <a href="/join" onClick={closeMobileMenu} className="inline-block rounded-full bg-slate-800 px-6 py-2 text-sm font-medium text-white shadow-sm hover:bg-slate-700 transition-colors">
                Dołącz do nas
              </a>
            </div>
          </div>
