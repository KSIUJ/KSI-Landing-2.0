...
export const Timeline: React.FC<TimelineProps> = ({
    ...
    const items = useMemo(() => {
        return events
            .map(e => ({ ...e, _date: parseDate(e.start_at) }))
            .filter(e => e._date !== null)
            .sort((a, b) => (a._date!.getTime() - b._date!.getTime()));
        }, [events]);

    if (!items.length) {
        return <div className="text-sm text-gray-500">Brak zaplanowanych wydarzeń.</div>;
    }

    // zakres od teraz do najdalszego eventu, z dodatkowym marginesem po prawej
    const rawMax = items.reduce((acc, it) => Math.max(acc, it._date!.getTime()), nowDate.getTime());
    const rawRange = Math.max(24 * 3600 * 1000, rawMax - nowDate.getTime()); // min 1 dzień
    const extra = Math.max(Math.floor(rawRange * rightMarginFraction), 24 * 3600 * 1000); // min 1 day extra
    ...
    <div className="w-full">
        ...
        <div style={{ "--my-mb": `${labelBaseOffsetPx + verticalStepPx * items.length}px` } as React.CSSProperties}
            ...
            {positioned.map((it, idx) => {
                ...
                const labelTop = `calc(50% + ${labelBaseOffsetPx}px + ${idx * verticalStepPx}px)`; 
        ...
        <div className="relative pl-6">
            ...
            {positioned.map((it) => {
                ...
                <div key={it.id} className="flex items-start gap-3">
                    ...
                    <div className="text-xs text-slate-500">
                        {distText} • {new Date(it._date!).toLocaleString("pl-PL", { day: "2-digit", month: "short", hour: "2-digit", minute: "2-digit" })}
                    </div>
                    </div>
                </div>
                );
